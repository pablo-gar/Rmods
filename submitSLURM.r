...
systemSubmit <- function(command, n = 12, ignore.stdout = TRUE, ignore.stderr = TRUE, intern = TRUE, ...) {
...
clusterSubmit <- function (jobName, outDir, commandList, queue = "hbfraser", time = NULL, qos = NULL, nodes = NULL, mem = NULL, deleteScript = FALSE, email = NULL, proc = NULL, totalProc = NULL, maxJobs = SLURM_MAX_JOBS, user = SLURM_USER){
	...
	if (!is.null(proc))	
		args <- c(args, paste0(arg , "--ntasks-per-node=" , as.character(proc)))
	...
	# Executing script
	systemSubmit(paste0("sbatch " , scriptName))
